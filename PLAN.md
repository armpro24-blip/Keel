...
- **T23 模型/serving 边界的工具协议失败（L2-R1，`docs/dogfood/L2/TOOL_PROTOCOL_AUDIT.md`，2026-09-09）**：第 40 次响应无可执行 `tool_calls` 也无可见文本，Keel 正确终止；隐藏 reasoning 以工具调用闭合标签结尾。责任待隔离。**决定（用户）**：先做实验机侧的有边界审计——(1) 记录实际启动参数、模型 revision、chat template、reasoning/tool parser、生成上限与停止配置；(2) 用只读脚本 `tools/audit_tool_protocol.py` 对第 40 次响应做结构化检查（是否完整 `<tool_call>` 块、工具名与参数名、是否为被拒编辑的重发、L2-R1 全部 40 次与 L1 会话中"reasoning 含工具标记"与"tool_calls 提取"的共现统计），只共享结构不公开正文；(3) 对照**实际安装版本**的官方实现确认通道划分与提取规则，区分模型输出不合规、配置不匹配、解析器缺陷；(4) 静态材料不能判别时再提交预注册的最小探针方案（正常调用、拒绝后重发两条件）先审批再执行。**不改 Keel、不改 PIRA、不重跑 L2、不授权从 reasoning 中提取并执行工具调用**：Keel 只执行正式 `tool_calls`，诊断可见性与执行恢复是两件事，目前不需要任一新机制。L2-R2 暂不批准；若找到并修复配置问题，窄范围验证后冻结新配置再申请；原 L2-R1 结果（10/13、未完成）永久保留；若未找到修复，是否做重复样本实验另行决定，不得用一次成功覆盖本次失败。**T22 结论**：实现成立——显式预算生效并越过 32、非预算错误未被误报为耗尽、未自动续接、日志对缺失计数诚实；任务级充分性：L2-R1 未触及预算，**L2-R2（2026-09-10）在第 100 次耗尽、任务未完成（11/13）**，即 100 次在该次运行中不足，而 L2 在两次 Continue 下 76 次完成；按预定规则不据此调高 N。
  **审计结果（2026-09-09，`docs/evidence/TOOL_PROTOCOL_AUDIT_2026-09-09.md`）**：步骤 1 部分可得（vLLM 在另一主机 192.168.3.103，启动参数、revision、模板哈希、max_tokens 不可得；`/version`、`/v1/models`、`/tokenize+/detokenize` 渲染出的模板、`/metrics` 缓存配置可得）；步骤 2 两份 wire 完成——77 次响应中 reasoning 含工具调用标记的恰为 4 次空回复（L1 第 18/26/38、L2-R1 第 40），签名一致：`finish=stop`、`tool_calls=0`、`content=''`、reasoning 有 `</tool_call>` 等闭合标签而无 `<tool_call>`/`<function=`/`</think>`；其余 73 次 reasoning 无标记且全部正常提取（含多调用与 18k 字符推理）；第 40 次不是被拒编辑的逐字节重发，而是同一修复线程中的下一次编辑（L2-R1 报告已勘误）。步骤 3 本机不可执行（无 vLLM 源码）。**分类：主因为模型输出不合规（思考通道未闭合即发出调用，违反模板"reasoning BEFORE the call, NOT after"）；次要待远端复核：服务端对不合规输出返回静默成功而非错误，开标签从 reasoning 文本中消失的去向决定解析器该记多少账。** 不属 Keel。协议校正：步骤 1 原假设 vLLM 与 Keel 同机，实际 Keel 在 192.168.3.182、vLLM 在 192.168.3.103；拆为 1a（Keel 主机 HTTP 侧，无需授权）与 1b（服务主机进程与源码侧，需独立远程访问授权，不从 Keel 会话发起 SSH——该尝试已被权限策略拦下）。**步骤 1b 与步骤 3 由实验机操作员在服务主机 vLLM 容器内只读完成（同日，交付包原样存于 `docs/dogfood/L2/audit_2026-09-09/`）。** 1b：`vllm/vllm-openai:v0.26.0`（构建 `ffd46bf`），`vllm serve nvidia/Qwen3.6-35B-A3B-NVFP4 … --reasoning-parser qwen3 --tool-call-parser qwen3_xml --enable-auto-tool-choice`，无 `--chat-template`（用权重自带模板，sha256 `e84f32a2…4259`，第 152 行生成提示以 `<think>` 开头），snapshot `1355db6a`，`generation_config` 为 `temperature 1.0 / top_k 20 / top_p 0.95`——Keel 不发采样参数，故迄今所有运行都用这组默认值；`<tool_call>`/`</tool_call>`/`<think>`/`</think>` 均为非特殊 token，detokenize 不会剥除。步骤 3（实验机源码阅读，行号经 Keel 侧与探针输出核对；源码本身 Keel 作者未读）：非流式路径唯一提取入口是 `serving.py:893-898` 的 `parser.parse(output.text, …)`，`ParserEngine.parse` 单遍扫描全文、初始态 REASONING；`(REASONING, TOOL_START) → TOOL_PREAMBLE` 迁移在本链路可达（`skip_tool_parsing` 只在 reasoning 适配器内置位，`_suppress_tool_calls` 只在 `tool_choice == "none"` 时置位，Keel 均不触发）；`_on_terminal` 的所有无迁移出路都把标记原文按当前状态回吐，唯一静默丢弃是 `DROP` 终结符；工具槽位只由 `TOOL_CALL_START` 创建。于是 reasoning 中缺失的 `<tool_call>`/`<function=`/首个 `<parameter=` 只能是从未生成：四次失败都是无头的调用尾巴接在思考散文之后。对照：L1 第 36 次（实验机口径 35）同样带游离 `</parameter>` 与臆造的 `</thinking>`，但写全了 `<tool_call>` 块，被正常提取。**最终分类：模型输出不合规；配置不匹配排除；解析器缺陷排除（静默成功是对"从未打开工具块的文本"的设计行为）。** 步骤 4 不需要。被推翻的中间假设（双适配器两阶段路径；解析器消费前缀后放弃）按预注册保留于实验机文稿附录 A。**待评审事项（不实现）**：实验机文稿第 6 节建议 Keel 侧按该签名（`stop`、无调用、空 content、reasoning 含 `</tool_call>` 无 `<tool_call>`）识别并重试该轮；与"空回复不自动重试"（T19）和"Keel 只执行正式 `tool_calls`、不读 reasoning"两项既有决定冲突，是否采纳由用户决定。复发风险仍未知（4/77 是计数不是率）。L2-R1 结果永久保留。**L2-R2（用户 2026-09-10 批准，仅一次）**：协议冻结于 `docs/dogfood/L2/README.md` "L2-R2"——全新种子仓库 `queuewatch-l2r2`，任务/验收/保全与 L2-R1 相同；Keel 运行时 822fdbc、PIRA 4e0682dd、模型 snapshot 1355db6a、vLLM 0.26.0 镜像与构建、解析器 `qwen3`/`qwen3_xml`、模板 sha256 `e84f32a2…`、采样为模型默认（Keel 请求体只有 model/messages/tools）均列为预检项，服务主机侧用审计探针 `step1b_fix.sh` 只读核对；`--full --trace --record-wire --max-turns 100`；零 Continue、不重试、不追加预算、预检不符即停、任何结束原因都不再跑；无论结果保留日志与最终 diff、执行验收与保全检查、报告结束原因/模型调用数/握手/人工干预；解读预先固定：通过记为"两次尝试中一次完成"不覆盖 L2-R1，空回复复发记为签名第二次出现且不引出任何 Keel 机制；证据写独立 `docs/evidence/L2R2_<date>.md`，然后停下评审。**L2-R2 结果（2026-09-10，`docs/evidence/L2R2_2026-09-10.md`）：预检全部相符；预算在第 100 次耗尽，任务未完成；隐藏验收 11/13（A01 模型自写测试的列宽期望与自身实现不符；A13 README 未动），种子 42/42 保全、新增 9；100 次响应全部 `finish=tool_calls`、reasoning 无任何工具标记、无空回复——审计签名 0/100 未复发；握手 27 次带评审先公告后执行、2 次缺评审被拒并恢复、排序违规 0；零 Continue、零提问。两次尝试两种不同的终止失败（协议失败 / 预算耗尽），均永久保留，互不覆盖。**
- **T24 调试绕路调查（`docs/dogfood/L2/DEBUG_TRACE_AUDIT.md`，2026-09-10，静态、只读）**：用户决定——接受 L2-R2 的失败分类；暂不继续完整取样、不提高预算、不实现空回复重试；下一步只分析已有日志。依据：T22 按设计工作但不证明预算足够；空回复不是唯一障碍（本次未复发任务仍未完成）；L2、L2-R1、L2-R2 都在模型自写的 `--status` 表格格式测试上反复修复，最后剩下的是模型自己的测试与实现不一致而非隐藏验收的要求。不能据此说"模型能力不足"或"PIRA 摘要导致失败"；调查聚焦**模型如何获得、理解和使用失败证据**。方法：先补齐 L2-R2 三项缺失材料；用新增只读脚本 `tools/debug_trace.py`（逐次模型调用时间线：工具、裁决、错误、`repeat xK`/`retrieve`/`helper`/结果 ID 机械标记，结果 ID 给出与检索对照；自检 `test_debug_trace.py`）还原从首次相关测试失败到第 100 次的过程；只回答三问：(1) 正确诊断所需信息是否出现在模型可见的工具输出里；(2) 若没有，是否保留在 `pira_ctx` 且返回可检索 ID、模型是否尝试检索；(3) 每次调用属于预注册六类之一（R 真实修复 / F 正常复现与反馈 / I 重复获取同一信息 / H 辅助脚本与 shell 文件机制 / P 读策略 / O 其他），34 次工具错误不全算浪费。预先固定的判读：信息未送到或不便获取 → 调查输出与检索接口；信息清楚送到仍反复误用 → 偏向模型调试行为；被 shell/辅助文件拖住 → 调查工具使用路径。判明后再选一个最小、通用的候选做对照实验；本调查不提议机制。预算 100、L2-R2 结果、冻结验收全部保留。**结果（2026-09-10，`docs/evidence/DEBUG_TRACE_2026-09-10.md`，实验机交付原样存于 `docs/dogfood/L2/debug_trace_2026-09-10/`）**：循环为第 66–100 次（35 次）。Q1 = yes：第 66 次以 exact 模式直接输出 162 行测试结果，含目标断言的期望行、实际行与 `?  ++` 差异标记，无摘要、无结果 ID；Q2 前件不成立，事实为全程 46 个结果 ID、0 次检索（L2 0/50、L2-R1 0/20 同样为零）；Q3 = R3 F8 I1 H22 P0 O1，`test_cli.py` 的期望在循环内一次未改，第 66 次后再未运行任何测试。H 的 22 次按机制：`python -c` 一行程序在无 shell 的 argv 下引号失败 4（全程 8）、借测试文件的 exec 技巧 2、辅助文件 `_check.py`/`_check2.py` 的创建/运行/编辑 16（其中 ModuleNotFoundError 3、CRLF 零匹配 2、评审拒绝 1、空 old_text 拒绝 1、辅助数据自身 bug 2）。循环外：20–30 用 11 次才以 exact 模式看到完整测试列表（摘要 unselected 39 行、一次 GNU find 遍历文件系统 231 MB、保留捕获可检索但未检索）；34–65 用 32 次把测试插入 `test_cli.py`（edit_file 锚点 `test_check_command_is_unaffected` 在种子中不存在、零匹配 4 次被当成字节问题、直接写文件把文件清空、git checkout 恢复、PowerShell here-string 成功）。跨运行：同一测试 L2 42–63 修好（改期望并重跑）、L2-R1 25–40 被空回复截断、L2-R2 66–100 耗尽预算。**按预定规则：规则 2（信息清楚送到、模型仍绕路）与规则 3（被 shell/辅助文件机制拖住）同时成立，规则 1 不成立**——模型看到了证据，却选择"重算期望字符串再比对"的验证路线，而这条路线的每一步都撞上在本工具契约下于 Windows 写与运行临时 Python 的机制。不提议机制；下一步由用户选一个最小、通用的候选做对照实验。工具修正：`debug_trace.py` 强制 UTF-8 输出（实验机曾因 cp1252 遇 `→` 中断，以 `PYTHONUTF8=1` 绕过）；`repeat` 标记改为只比较命令（argv+mode / path+texts）而不含 intent 文本，交付的时间线用的是旧口径。
- **T25 `shell` 工具参数传递契约的澄清（`docs/dogfood/shell_contract/SHELL_CONTRACT_AB.md`，2026-09-10，用户选定的对照实验候选）**：T24 分出两个问题——调试路线绕远（PIRA 方法与模型行为，现在不往 Keel 塞"如何调试"的规则）与绕远后不断撞上工具使用机制（引号、临时文件、导入路径、CRLF，属于 Keel 自己负责说明的工具契约）。候选只改实验中的 `shell` 工具描述：argv 每个元素是一个参数而非一整条终端命令；调用普通程序时不要加仅供 shell 分组的外层引号、内容自身的引号保留；显式调用 shell 后才进入该 shell 的解析规则；`workdir` 只是进程工作目录、不改变被调用程序自身的模块搜索规则。不加入 queuewatch、测试名、列宽、期望字符串或"改断言即可"——是接口说明不是解题策略；不新增工具、不改执行语义、不重跑完整 L2；不期待它纠正不存在的编辑锚点或保证模型改对测试。**先过一道确定性检查（阶段 1）**：`tests/shell_transport.rs`（默认 `#[ignore]`，在有 PIRA 的机器上显式运行）把含空格、嵌套引号、反斜杠、空串、结尾反斜杠的固定 argv 经真实路径 `wrap_command → pira_ctx … -- python … → run_process` 送到子进程并比对收到的参数；若传输改变了合法参数，停止描述实验、转为修复传输缺陷，不能用更详细的提示掩盖实现缺陷。细读 L2-R2 时间线后把"引号失败"拆为两类：显式 `cmd /C` 对含引号元素的二次解析（39、54、92，描述可以说明）与模型写出的非法一行 Python（`;` 后接 `try:`/`def`，68、80，非传输非引号，描述不教 Python、只说明参数可含换行）。**阶段 2（草案待评审，未批准运行）**：A 现有描述 / B 澄清描述（B 只改字符串、放实验分支、两份二进制），模型、采样、执行器、权限、每次预算 `--max-turns 12` 完全相同；4 个冻结小任务（读代码算确定结果 R1 测试方法数 42、R2 `report.py` 的 sha256；含引号/空格/反斜杠的数据 D1 按字节写入一行、D2 从 `data/quotes.jsonl` 取出 id=3 的 name）× 2 臂 × 10 次交错 = 80 次；主要结局为正确性、参数使用错误运行数（argv 坍缩、shell 操作符拒绝、shell 二次解析类 SyntaxError、参数被改变）与辅助文件修复调用数，非法一行 Python 只作信息不计入任一臂，总调用数只报告不判决；判断门槛（拟）：B 正确数 ≥ A、参数使用错误运行数 ≤ A 的一半、辅助文件调用 ≤ A、无任务类别更差；A 的参数使用错误运行不足 4 次则判为无法判别而非无效。B 不稳定改善即停止该候选；通过后才讨论合入与完整任务回归。**阶段 1 结果（2026-09-10，`docs/evidence/SHELL_TRANSPORT_2026-09-10.md`）**：作者机（pira_ctx 1.8.0 / Python 3.14）与实验机（1.9.0 / 3.12.10）五项测试全部通过、观察逐行相同——合法参数在 exact 与默认模式下逐字节到达，含两种引号、反斜杠与换行的 `-c` 程序原样执行；停止规则未触发，传输不是问题。L2-R2 第 68、74、77、80 次经回溯回显核对，四次全部是非法一行 Python（`;` 后接 `try:`/`def`，74/77/80 的回显与发出程序逐字节相同），无一属于 shell 二次解析；后者在 L2-R2 中是 39、54、92（`cmd /C`），第五项探针复现其形状。阶段 2 草案按实验机意见修订（待用户批准）：D1 任务文本写明 LF、无 BOM，行尾/BOM 差异单列；R2 的正确性依赖种子 `.gitattributes` 与 `init_l2.sh` 的 autocrlf 设置，以运行仓库的磁盘文件评分；shell 二次解析与非法一行 Python 按 argv 是否调用 shell 机械区分；辅助文件调用按任务分层报告；门槛 (2) 取 ⌊A/2⌋；B 文本对重定向表态（独立参数拒绝、仅在显式 shell 的命令元素内有效）。**阶段 2 批准（用户，2026-09-10，仅一次）附三处修正**：(1) B 文本删去重复的"程序文本自身含引号时同样成立"，`workdir` 改为"设置子进程当前工作目录，相对路径与模块导入随后按该程序自身规则解析"（原稿"不改变相对路径"易误读）；(2) 错误分类改为三类——已确认参数使用错误（结构拒绝，或输出证据表明显式 shell 改变/拆分了元素，如回显行不是任何已发送元素的一行）、程序自身错误（回显行是已发送元素的一行，经 shell 亦然；不计入参数错误但影响正确性与调用成本，按臂报告）、无法判别（无回显行；不得并入第一类）；"helper-file repair calls"改名"辅助文件相关调用"，计数定义不变，辅助脚本本身不是错误；(3) 每次至多 12 次模型调用，**不回答模型提问**（提问即结束、记未完成、保留提问文本，否则计数重置），正确性要求正常结束且通过结果检查（D1 文件正确但预算耗尽记为"文件正确、运行未完成"）。规模接受：4×2×10、每次 12 次、总上限 960 次调用，"约 1.5 小时"只是估算；每个任务内 AB/BA 交替。三条门槛保留但改称"预注册的候选推进门槛"而非"稳定改善"；A 不足 4 次已确认参数错误即如实报无法判别、不追加样本。B 分支 `exp/shell-contract-b`（只改 `src/shell.rs` 描述字符串，`cargo test` 通过）；评分脚本 `tools/score_shell_ab.py`（每次一行 CSV：结束原因、正确、D1 文件状态、三类错误计数、辅助文件路径清单、握手计数；自检 `test_score_shell_ab.py`）。**这次最重要的进展是纠正了自己的诊断**：许多原以为是引号传递的问题实为模型写了非法程序，候选可以测，但对其能解决多少问题降低预期。**阶段 2 结果（2026-09-10，`docs/evidence/SHELL_CONTRACT_AB_2026-09-10.md`，实验机交付原样存于 `docs/dogfood/shell_contract/stage2_2026-09-10/`）：80/80 完成，B 未达候选推进门槛，候选停止。** 条件 (1) 总正确 B 39 ≥ A 34 但 R1 上 B 9 < A 10；条件 (2) A 已确认参数错误运行 5（可判别）、B 9 > ⌊5/2⌋=2；条件 (3) 修正后辅助文件调用 A 1 / B 1 但 D1 上 B 1 > A 0（交付原值 A 2 / B 8，其中 7+1 为评分脚本把已跟踪文件误列为 helper，已修脚本、原 CSV 不改）。18 个已确认参数错误事件全部是 `argv` 被当作字符串的结构拒绝，B 反而更多（12 vs 6）；B 针对的 shell 二次解析类在 80 次中 0 次出现，实验未能观察到 B 所针对的效应。A 的 6 次不正确中 3 次是答案外包了散文/反引号、2 次是 D1 字节精确（缺 LF；CRLF/BOM）、1 次预算耗尽；B 的 1 次是 R1 答 8。B 调用更多（202 vs 180，主要 D1 的多次十六进制核验）。握手 80/80 无排序违规、`approve?` 0、无提问、干预仅任务行与 /quit。按预注册规则不合入描述、`exp/shell-contract-b` 保留为记录、运行时仍为 822fdbc；argv 坍缩类是否值得另立候选是新决定而非本候选的延续。
- **T26 跨模型 L2 对照（`docs/dogfood/L2/CROSS_MODEL_L2.md`，2026-09-10，仅批准准备）**：用户接受本阶段收口后转向——用一个不同模型家族的模型做一次冻结的 L2 对照，回答"当前完成能力的限制在多大程度上与被测模型配置有关，而不是 Keel 的通用工具设计"。边界：Keel 保持 822fdbc，PIRA、L2 任务、验收、100 次预算、零 Continue 不变；不为对照模型添加专用提示、工具描述或恢复机制；先确认部署与工具协议兼容（T15 五项探针），再冻结运行协议；serving 差异如实记录不拉平。无论成败，单次结果不得用来宣布某模型"更强"或 Keel "没有问题"。**现在只批准准备：实验机列出已可用候选模型、部署方式与预计成本，用户选一个，不做排行榜；不批准运行或购买算力。**
...
**L2-R2 协议冻结（2026-09-10，`docs/dogfood/L2/README.md` "L2-R2"）：用户批准一次重复运行，条件与 L2-R1 完全相同，作为同条件下的第二个样本。** 新增的预检来自审计事实：模型 snapshot、模板与 generation_config 哈希、解析器与镜像/构建提交在服务主机只读核对，Keel 请求体无采样参数在 wire 上事后核对；任何结束原因（完成、Keel 报错、预算耗尽）都终止实验且不重跑；两次结果按序列报告，L2-R1 不被覆盖。运行结果待实验机回报。

**L2-R2 结果（2026-09-10，`docs/evidence/L2R2_2026-09-10.md`）：失败——预算耗尽。** 预检全部相符（运行时 822fdbc、PIRA 4e0682dd、snapshot 1355db6a、模板/generation_config 哈希、解析器、镜像与构建、请求体键 `['messages','model','tools']`）。第 100 次调用后 Keel 打印 `error: model-call budget exhausted (max_turns = 100); the run is incomplete; the transcript is kept`，零 Continue、零提问、`/quit` 唯一操作。隐藏验收 11/13（A01：项目 51 测试 1 失败，是模型自写的 `--status` 列宽期望与自身实现不符；A13：README 未动；L2-R1 失败的 A10 本次通过）；种子 42/42 保全、新增 9；遗留 `tests/_check.py`、`_check2.py`、`_write_cli.py`、`tests_output.txt`。123 次工具调用（shell 94、read_pira_policy 6、edit_file 23）、34 次工具错误（6 次 edit_file 零匹配，其中 2 次为 CRLF；其余为模型自测与辅助脚本的非零退出/stderr）；上下文至 73.2k、prompt 总和 4.54M = 前缀缓存查询增量、命中率 0.914、TTFT 1.340 s、9.6 分钟。**审计签名 0/100 未复发**：全部响应 `finish=tool_calls`，reasoning 无工具标记。握手 27/27 先公告后执行、2 次缺评审被拒后恢复、1 次 argv 坍缩被拒后重发、排序违规 0。**主导失败分类**：终止性失败是预算耗尽（Keel 按设计工作，T22 任务级结论收紧为"100 次在本次不足"，不调 N）；模型侧为对 `--status` 表格格式与 Windows shell/文件机制的低效自修复，耗尽预算前未到 README。`l2r2_diff.patch` 余部、`pira_ctx history` 尾部、logshow 行数因传输截断待补，不影响分类。两次尝试两种终止失败，均永久保留。冻结证据、停下评审；未选机制。

**决定（用户，2026-09-10）与 T24 协议冻结**：接受 L2-R2 分类；不再取样、不调预算、不实现重试；下一步是对已有日志的静态调试轨迹审计（`docs/dogfood/L2/DEBUG_TRACE_AUDIT.md`）——"现在不是给它更多次数，而是先看清它为什么花掉这么多次数"。三问（信息是否可见 / 是否可检索且尝试检索 / 每次调用的用途分类）与判读规则预先固定；结果待实验机回报。

**T24 结果（2026-09-10，`docs/evidence/DEBUG_TRACE_2026-09-10.md`）：信息已送到（Q1 yes）；模型未检索任何保留捕获（0/46）；循环 35 次中 22 次花在辅助脚本与 shell/文件机制上（Q3 H22 R3 F8 I1 O1）。** 第 66 次的直接输出已完整显示模型自写期望的错误，模型修了实现侧两处与 test_report 期望，却对 test_cli 的期望一次未改、此后再未运行测试，转而用 `python -c` 一行程序与 `_check.py` 辅助脚本去重算期望字符串，依次撞上无 shell 的 argv 引号、`python 文件` 运行时包不可导入、text 模式写文件得到 CRLF 使 edit_file 零匹配。循环外还有两段同类机制消耗：20–30（11 次，摘要与 exact 模式）与 34–65（32 次，锚点不存在的 edit_file 与直接写文件）。预定规则 2 与 3 成立、规则 1 不成立；不提议机制，等用户选对照实验候选。实验机交付原样存于 `docs/dogfood/L2/debug_trace_2026-09-10/`。

**T25 候选选定与阶段 1 冻结（用户，2026-09-10）**：对照实验候选是澄清 `shell` 工具的参数传递契约（只改描述、不新增工具、不改执行语义、不重跑完整 L2）。先做确定性传输检查 `tests/shell_transport.rs`（传输改变合法参数则停止实验转修传输）；A/B 协议草案写入 `docs/dogfood/shell_contract/SHELL_CONTRACT_AB.md` 供评审，未批准运行模型。直观解释：暂不教它"怎么修这道题"，先验证"工具的用法是否讲得足够清楚"——这是 harness 的本职。

**T25 阶段 1 完成（2026-09-10）：传输不是问题。** 两台机器五项传输测试全过、观察逐行相同；L2-R2 四次 SyntaxError 全为非法一行 Python，shell 二次解析仅见于显式 `cmd /C` 的三次。阶段 2 草案按实验机评审意见修订完毕，等待用户批准 B 文本与门槛后方可运行。

**T25 阶段 2 批准并冻结（用户，2026-09-10）**：三处修正后批准一次 A/B（B 文本两句改动；错误三分类，不把 Python 错误算成 shell 错误；不回答提问、正确性须正常结束）。规模 4×2×10、每次 12 次、AB/BA 交替；门槛改称"候选推进门槛"，A 不足 4 次不追加样本。B 在分支 `exp/shell-contract-b`；评分脚本就位。不改运行时、不改任务提高出错率；结果出来后停下。

**T25 阶段 2 结果（2026-09-10，`docs/evidence/SHELL_CONTRACT_AB_2026-09-10.md`）：候选停止。** 80/80 运行，382 次模型调用；B 正确 39/40 vs A 34/40，但三条门槛均未全部满足（R1 上 B 退步一次；B 的已确认参数错误运行 9 > A 5 的一半；D1 上 B 多一次辅助文件调用）。所有已确认参数错误都是 argv 被当作字符串的结构拒绝，B 文本针对的 shell 二次解析在 80 次中一次未出现，实验未能测到 B 的目标效应；B 的正确率优势在本样本内真实但无法归因于两句改动。不合入、不改运行时；评分脚本修了一处已跟踪文件误判（不影响判定）。

**决定（用户，2026-09-10）：收口，不再追着单个错误加机制。** T25 停止后不新增实验、不新增运行时代码；由实验机写一份简短阶段总结，只回答三件事：(1) 已验证的能力——预算与权限执行、错误可见性、转录保留，以及限定工作负载下的成功案例（Gate D 10/10、L1-R1 7/7、L1-R2 7/7、L2 13/13 带两次 Continue、T25 小任务 A 34/40 B 39/40）；(2) 仍未达到的目标——当前模型配置下 100 次预算、零 Continue 的 L2 自主完成尚未成功（L2-R1 空回复终止 10/13，L2-R2 预算耗尽 11/13）；(3) 已否决或未证实的方向——工具描述 B 停止；自动恢复（空回复重试、reasoning 提取）未批准；模型协议失败（T23）、调试绕路（T24）与工具参数错误（T25）是三个不同的现象，不得混为一个根因。总结存入 `docs/evidence/PHASE_SUMMARY_<date>.md`，然后停下。

**阶段总结存档（2026-09-10，`docs/evidence/PHASE_SUMMARY_2026-09-10.md`）**：实验机交付原文原样存入，作者逐项核对数字与引用，一处措辞（"每次都在下一次调用被纠正"，源自作者的 T25 证据）超出可核实范围，已在附注中改为可核实表述并同步修正 `SHELL_CONTRACT_AB_2026-09-10.md`；结论未改。本阶段到此收口。

**决定（用户，2026-09-10）：接受收口；下一阶段转向跨模型 L2 对照（T26），只批准准备。** 不再调整当前模型的提示、预算或工具描述；实验机先列候选模型、部署方式、预计成本；项目从"解释当前模型为什么失败"转向"验证 Keel 的设计能否跨模型成立"。

**实际使用 #1（2026-09-11，用户批准并确认）：通过 Keel（ask 模式，默认预算 32）更新 README 运行段。** 成果：启动示例四行、`--max-turns` 语义一段、`--record-wire` 指向 M3 段一句，+8/−1，数值与 `src/cli.rs`/`src/agent.rs` 核对一致，提交前操作员把引号内写死的 `32` 改为 `N`。代价：42 次模型调用、2 条用户消息（第一条预算耗尽；一条人工提示后第二条 10 次完成）、审批 45 同意 / 3 拒绝（2 次人工、1 次结构校验 `intent` 缺失）、编辑 5 次成功 2 次。阻碍：长文件经 pira_ctx 摘要截断，模型用 12 次调用换读法才用到 `range`；从控制台渲染复制 old_text 导致零匹配 2 次（`—` 失真、换行合并）、公共前缀多匹配 1 次；提示"用 exact 模式读原始字节"后一次命中。这是"长输出被摘要 → 反复重读"的第二次出现（首次 T24 第 20–30 次）；按收口后的规则记录不修，第三次出现再提一个最小修复。不新增机制、不写证据报告。
